...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "postgresql://posts_nurn_user:MFN5C5wM9iEM6bSUhkT3hMckOyhnhv38@dpg-cvhcs0dds78s73e43t2g-a/posts_nurn"  # PSQL for live
)
...
class Post(db.Model):
    ...
    user = db.relationship(
        "User", backref=db.backref("posts", lazy=True), foreign_keys=[user_id]
...
class Likes(db.Model):
    ...
    post = db.relationship(
        "Post", backref=db.backref("post", lazy=True)
    )  # 🔹 Relationship to User model
...
)  # Allow specific headers for POST requests
def sign_up():
    data = request.json
...
def get_posts():
    ...
    print(
        f"Received user_id from JWT: {user_id}"
    )  # Log the user_id to see if it's correctly decoded
    ...
    for post in all_posts:
        like_count = Likes.query.filter_by(
            post_id=post.id
        ).count()  # Count likes for the post
...
def create_comment():
    ...
    new_comment = Comment(
        content=data["content"], user_id=user_id, post_id=data["post_id"]
    )
    ...
    if "profile_picture" in request.files:
        ...
        if profile_picture and allowed_file(profile_picture.filename):
            ...
            profile_path = os.path.join(
                app.config["UPLOAD_FOLDER"], "profile_pics", filename
            )
        ...
        if data["banner_type"] == "color":
            ...
            if banner and allowed_file(banner.filename):
                ...
                banner_path = os.path.join(
                    app.config["UPLOAD_FOLDER"], "banners", filename
                )
...
def admin_dashboard():
    ...
    def protected_dashboard():
        ...
        chart_data = [
            {"date": date, "count": posts_by_day.get(date, 0)} for date in date_range
        ]
